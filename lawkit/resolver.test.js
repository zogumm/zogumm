...
/**
 * 이게 "뼈대"인지 판별하는 유일한 기준이다.
 * 새 특별법 지구를 붙일 때 resolver.js 를 열어야 한다면 뼈대가 아니다.
 *
 * 아래 룰은 구조 검증용 합성 데이터다. 실제 법령값이 아니므로 rules/ 에 넣지 않는다.
 */
const 합성룰 = {
  id: 'test-synthetic',
  법령: '가상 특별법 시행령',
  ...
  적용조건: { 구역: '공공주택지구' },
  트랙: [
    { 용도: '공동주택', 자격: '*', 건폐율: 50, 용적률: 250 },
    { 용도: '그외', 자격: '*', 건폐율: 30 },
  ],
};

test('확장 — 새 지구는 룰 파일 하나로 붙는다 (코드 수정 없음)', () => {
  const 대지2 = { 대지면적: 1000, 용도지역: '제2종일반주거지역', 구역: ['공공주택지구'] };

  // 룰 없이는 보류
  assert.equal(resolve({ ...대지2, 용도: '공동주택' }, { rules }).status, STATUS.NEEDS_REVIEW);

  // 룰을 얹으면 그대로 계산된다
  const r = resolve({ ...대지2, 용도: '공동주택' }, { rules: [...rules, 합성룰] });
  assert.equal(r.status, STATUS.OK);
  assert.equal(r.건폐율, 50);
  assert.equal(r.최대건축면적, 500);
  assert.equal(r.최대연면적, 2500);
  assert.match(r.근거.join(' '), /제99조/);
});

test('확장 — 룰이 여러 개여도 해당 지구 것만 매칭된다', () => {
  const all = [...rules, 합성룰];

  const gb = resolve(
    { ...대지, 용도: '단독주택', 자격: '지정당시거주자' }, { rules: all },
  );
  assert.equal(gb.건폐율, 60, 'GB 필지는 합성룰에 영향받지 않는다');

  const 공공 = resolve(
    { 대지면적: 1000, 용도지역: '제2종일반주거지역', 구역: ['공공주택지구'], 용도: '그외' },
    { rules: all },
  );
  assert.equal(공공.건폐율, 30);
});

test('확장 — 검수완료 룰은 미검수 경고를 달지 않는다', () => {
  const r = resolve(
    { 대지면적: 1000, 용도지역: '제2종일반주거지역', 구역: ['공공주택지구'], 용도: '공동주택' },
    { rules: [합성룰] },
  );
  assert.equal(r.경고.filter((w) => w.includes('미검수')).length, 0);
});
